...
export function findNearestColorName(hex: string): { name: string; hex: string; distance: number } {
  let best = { name: "black", hex: "#000000", distance: Infinity };
  for (const [name, namedHex] of Object.entries(CSS_NAMED_COLORS)) {
    const dist = chroma.deltaE(hex, namedHex);
    if (dist < best.distance) {
      best = { name, hex: namedHex, distance: Math.round(dist * 10) / 10 };
    }
  }
  return best;
}
